...
var noRepeatPointMsg = function(username){
  return `@${username}, you have already given a :musical_note: for this song`;
};

var badFormatMsg = function(username){
  return `@${username} you need to use @[username] to give a :musical_note: to someone`;
};

var noMultiAwarding = function(username){
  return `@${username} you can only give a :musical_note: to one person`;
};
